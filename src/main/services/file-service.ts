...
export class FileService {
  ...
  private readonly inflightReads = new Map<string, { revision: number; promise: Promise<string | Buffer> }>();
  private cacheBytes = 0;
  private mutationRevision = 0;
  private readonly mutationRevisionsByPath = new Map<string, number>();
  private readonly mutationListeners = new Set<(event: FileMutationEvent) => void>();
  private readonly atomicMutationQueues = new Map<string, Promise<void>>();
  ...
  async readText(path: string): Promise<string> {
    ...
    const revision = this.revision(absPath);
    const inflight = this.inflightReads.get(key);
    if (inflight?.revision === revision) return inflight.promise as Promise<string>;

    const promise = this.readAndCacheText(absPath, revision);
    this.inflightReads.set(key, { revision, promise });
    try {
      return await promise;
    } finally {
      if (this.inflightReads.get(key)?.promise === promise) this.inflightReads.delete(key);
    }
  }
  ...
  async readBuffer(path: string): Promise<Buffer> {
    ...
    const revision = this.revision(absPath);
    const inflight = this.inflightReads.get(key);
    if (inflight?.revision === revision) return inflight.promise as Promise<Buffer>;

    const promise = this.readAndCacheBuffer(absPath, revision);
    this.inflightReads.set(key, { revision, promise });
    try {
      return await promise;
    } finally {
  ...
  async writeText(path: string, value: string): Promise<void> {
    ...
    this.emitMutation(absPath, false);
  }
  ...
  async writeBufferAtomic(path: string, value: Buffer | Uint8Array): Promise<void> {
    const absPath = normalizePath(path);
    await this.withAtomicMutation(absPath, async () => {
      this.markMutation(absPath);
      await this.ensureDir(dirname(absPath));
      const tmpPath = `${absPath}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`;
      try {
        await fsWriteFile(tmpPath, value);
        await fsRename(tmpPath, absPath);
      } catch (error) {
        await fsRm(tmpPath, { force: true }).catch(() => undefined);
        throw error;
      }
      this.markMutation(absPath);
  ...
  async appendTextAtomic(path: string, value: string): Promise<void> {
    const absPath = normalizePath(path);
    await this.withAtomicMutation(absPath, async () => {
      this.markMutation(absPath);
      await this.ensureDir(dirname(absPath));
      ...
      try {
        try {
          await fsCopyFile(absPath, tmpPath);
        } catch (error) {
          if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
          await fsWriteFile(tmpPath, "", "utf8");
        }
        const handle = await fsOpen(tmpPath, "a");
        try {
  ...
  private async withAtomicMutation<T>(path: string, mutation: () => Promise<T>): Promise<T> {
    const previous = this.atomicMutationQueues.get(path) ?? Promise.resolve();
    const current = previous.catch(() => undefined).then(mutation);
    const tail = current.then(() => undefined, () => undefined);
    this.atomicMutationQueues.set(path, tail);
    try {
      return await current;
    } finally {
      if (this.atomicMutationQueues.get(path) === tail) this.atomicMutationQueues.delete(path);
    }
  }
  ...
  async copy(sourcePath: string, targetPath: string): Promise<void> {
    ...
    this.emitMutation(absTarget, false);
  }
  ...
  async rename(sourcePath: string, targetPath: string): Promise<void> {
    ...
    this.invalidate(absTarget);
    this.emitMutation(absSource, false);
    this.emitMutation(absTarget, false);
  }
  ...
  subscribeMutations(listener: (event: FileMutationEvent) => void): () => void {
    this.mutationListeners.add(listener);
    return () => this.mutationListeners.delete(listener);
  }

  noteExternalMutation(path: string): void {
    const absPath = normalizePath(path);
    this.markMutation(absPath);
    this.invalidate(absPath);
    this.emitMutation(absPath, true);
  }
  ...
  private async readAndCacheText(path: string, revision: number): Promise<string> {
    const value = await fsReadFile(path, "utf8");
    if (this.revision(path) === revision) await this.rememberText(path, value);
    return value;
  }

  private async readAndCacheBuffer(path: string, revision: number): Promise<Buffer> {
    const value = await fsReadFile(path);
    if (this.revision(path) === revision) await this.rememberBuffer(path, value);
    return value;
  }
  ...
  private emitMutation(path: string, external: boolean): void {
    const event = { path: normalizePath(path), external };
